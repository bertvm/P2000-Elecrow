...
void drawHeading(String text, int x, int top, int width, uint8_t scale = 1, uint16_t colour = UI_TEXT) {
...
void drawTwoLineHeading(String text, int x, int top, int width, uint16_t colour) {
  text.replace("\r", " "); text.replace("\n", " ");
  while (text.indexOf("  ") >= 0) text.replace("  ", " ");
  gfx->setFont(&FreeSansBold10pt7b); gfx->setTextSize(1); gfx->setTextWrap(false); gfx->setTextColor(colour);
  for (uint8_t lineNumber = 0; lineNumber < 2 && text.length(); ++lineNumber) {
    String line = text;
    int16_t bx, by; uint16_t bw, bh;
    gfx->getTextBounds(line, 0, 0, &bx, &by, &bw, &bh);
    if (bw > width) {
      size_t cut = line.length(), lastSpace = 0;
      while (cut > 0) {
        String candidate = line.substring(0, cut);
        gfx->getTextBounds(candidate, 0, 0, &bx, &by, &bw, &bh);
        if (bw <= width) break;
        --cut;
      }
      for (size_t i = 0; i < cut; ++i) if (line[i] == ' ' || line[i] == '|') lastSpace = i + 1;
      if (lastSpace > cut / 2) cut = lastSpace;
      line = text.substring(0, cut); line.trim();
      text.remove(0, cut); text.trim();
    } else text = "";
    if (lineNumber == 1 && text.length()) {
      while (line.length() && line.length() > 3) {
        gfx->getTextBounds(line + "...", 0, 0, &bx, &by, &bw, &bh);
        if (bw <= width) break;
    ...
    gfx->getTextBounds(line, 0, 0, &bx, &by, &bw, &bh);
    gfx->setCursor(x - bx, top + lineNumber * 24 - by); gfx->print(line);
  }
  gfx->setFont(); gfx->setTextSize(1);
}
void drawUiHeader(const char *title, uint8_t regionMaxChars = 40) {
  ...
  // WiFi veldsterkte: vier balken tussen de schermtitel en Config.
  ...
  constexpr int wifiX = 525, statusBaseY = 49;
  for (int i = 0; i < 4; ++i) {
    int height = 6 + i * 6, x = wifiX + i * 11;
    uint16_t colour = i < bars ? UI_ACCENT : UI_MUTED;
    if (i < bars) gfx->fillRoundRect(x, statusBaseY - height, 7, height, 2, colour);
    else gfx->drawRoundRect(x, statusBaseY - height, 7, height, 2, colour);
  }

  // API-verbinding als stekkersymbool: grijs voor nog niet getest, groen
  // voor bereikbaar en rood na een mislukte request.
  uint16_t apiColour = !apiChecked ? UI_MUTED : (wifiUp && apiConnected ? UI_AMBULANCE : UI_FIRE);
  constexpr int apiX = 592, apiY = 25;
  gfx->drawRoundRect(apiX, apiY, 24, 20, 5, apiColour);
  gfx->drawFastVLine(apiX + 7, apiY - 6, 8, apiColour);
  gfx->drawFastVLine(apiX + 17, apiY - 6, 8, apiColour);
  gfx->drawFastVLine(apiX + 12, apiY + 20, 8, apiColour);
  gfx->fillCircle(apiX + 12, apiY + 29, 2, apiColour);
...
String headerSignature(const char *title, uint8_t regionChars) {
  return String(title) + "\x1f" + selectedRegionsLabel(regionChars) + "\x1f" +
         String((int)WiFi.status()) + "\x1f" + String(WiFi.RSSI() / 5) + "\x1f" +
         String(apiChecked) + String(apiConnected);
}
...
void drawScreen() {
  if (cfg.ticker && !archiveMode) {
    ...
    if (fullRedraw || currentInfo != lastInfoSignature) {
      ...
      drawHeading(a.time + "  |  " + a.caps, 98, 151, 654, 1, accent);
  ...
  for (uint8_t visible = 0; visible < VISIBLE_ALARM_CARDS; ++visible) {
    ...
    int y = 108 + visible * 160;
    // This rectangle fully covers the old rounded card without touching neighbours.
    gfx->fillRect(20, y - 2, 760, 154, UI_BG);
    if (index < shownCount) {
      Alarm &a = shownAlarms[index];
      uint16_t accent = alarmAccent(a);
      gfx->fillRoundRect(24, y, 752, 150, 12, UI_SURFACE);
      gfx->fillRoundRect(24, y, 8, 150, 4, accent);
      gfx->fillCircle(66, y + 75, 27, accent);
      drawServiceIcon(serviceIcon(a), 50, y + 59);
      drawHeading(a.place.length() ? a.place : a.region, 106, y + 18, 646);
      drawTwoLineHeading(a.time + "  |  Capcode " + a.caps, 106, y + 50, 646, UI_ACCENT);
      drawHeading(a.text, 106, y + 106, 646, 1, UI_MUTED);
    }
...
void handleTap(int x, int y) {
  if (screenMode == MAP_DETAIL) {
    if (y <= 70) { mapJob.phase = MAP_LOAD_IDLE; screenMode = MESSAGES; invalidateMessageUi(); drawScreen(); }
    return;
  }
  ...
  if (readTouch(x, y)) {
    ...
    if (!touchPressed) {
      ...
      if ((!cfg.ticker || archiveMode) && abs(y - touchStartY) >= 50) {
        if (archiveMode) {
          if (y < touchStartY) {
            if (archiveFirstVisible + VISIBLE_ALARM_CARDS < archiveAlarmCount) ++archiveFirstVisible;
            else {
...
void pollApi() {
  ...
  http.setUserAgent("P2000-Elecrow-Map/2.0.1");
...
void loop() {
  ...
  if (cfg.feedMode != FEED_MQTT && WiFi.status() == WL_CONNECTED && screenMode != MAP_DETAIL && millis() > nextPoll) {
    nextPoll = millis() + cfg.intervalSec * 1000UL; pollApi();
  }
