...
- Meldingen: tabs voor drie regio's, vijf diensten, capcodes en **Bron**. Bij Bron kies je Alarmeringdroid, de lokale HTTP-API van [P2000-server](https://github.com/bertvm/P2000-server) (`http://<pi>:8080/api2/find/`) of MQTT (`p2000/alerts` op poort 1883). Vul het IP of de hostname van de Pi in. MQTT-gebruiker en -wachtwoord staan op de webpagina.
- Weergave: selecteer de lijst of het infoscherm en wijzig het API-interval in stappen van 15 seconden (niet van toepassing op MQTT).
...
4. Test capcodes invoeren/wissen, de bron (cloud / lokale API / MQTT) en de lijst-/infoselectie.
